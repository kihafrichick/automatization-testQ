...
class CriticalTestError(Exception):
    """Исключение для критических ошибок"""
    pass

class FunctionGroupTester:
    def __init__(self, selected_groups: Set[int] = None):
        self.function_pairs = [
            (module1.initialization, module2.initialization_blade, 1381),
    ...
    async def _update_kiwi_status(self, test_case_id: int, passed: bool, details: list):
        ...
        try:
            ...
            logger.info(f"Kiwi обновлен: TestCase {test_case_id} -> {status}")
...
def selected_groups():
    """Получаем выбранные группы из переменных окружения"""
...
async def test_function_group(group_idx, selected_groups):
    ...
    if not result['passed']:
        error_details = f"Группа {result['group']} провалена: {' | '.join(result['details'])}"
