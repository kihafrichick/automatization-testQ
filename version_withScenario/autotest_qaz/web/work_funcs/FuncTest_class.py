...
class FunctionGroupTester:
    ...
    async def _execute_single(self, func: Callable) -> bool:
        ...
        try:
            # Перехватываем stdout для сбора логов (нужно для Kiwi)
            ...
            # Всегда возвращаем stdout на место
            sys.stdout = original_stdout
            captured_output = output_buffer.getvalue()
            
            if captured_output:
                # Сохраняем логи для последующего использования в details/Kiwi
                lines = [line.strip() for line in captured_output.split('\n') if line.strip()]
                self.all_execution_logs.extend(lines)
                # В обычном режиме сразу выводим в консоль, чтобы пользователь видел логи
...
#Обновление статуса в Kiwi
        try:
            ...
            if passed:
                ...
                status = 'FAILED'
                # Формируем комментарий из details, если они есть
                if details:
                    comment = f"{' | '.join(details)}"
                else:
                    # Если details пустой, но статус FAILED, значит была ошибка без явного сообщения
                    comment = "Группа завершилась с ошибкой (детали недоступны)"
